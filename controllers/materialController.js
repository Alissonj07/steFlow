...
exports.listarMateriais = (req, res) => {
  res.json(tenis);
};

exports.criarMaterial = (req, res) => {
...
exports.buscarMaterial = (req, res) => {
  const { nome } = req.query;
  const resultadoBusca = tenis.filter((t) =>
...
exports.atualizarEstoque = (req, res) => {
  ...
  if (!tenisEncontrado) {
    return res.status(404).json({ mensagem: 'Tênis não encontrado' });
  }
